...
def test_browse_dir_click_cancels_does_not_clear_textbox(monkeypatch):
    ...
    monkeypatch.setattr(gptscan.tkinter.filedialog, 'askdirectory', lambda **kwargs: '')
...
def test_set_scanning_state_updates_buttons(monkeypatch):
    ...
    mock_scan_button.config.assert_any_call(text="Stop Scan")
    mock_scan_button.config.assert_any_call(state="normal")

    # Test scanning=False
    gptscan.set_scanning_state(False)
    mock_scan_button.config.assert_any_call(text="Scan Now")
    mock_scan_button.config.assert_any_call(state="normal")
...
def test_button_click_cancels_if_already_running(monkeypatch):
    ...
    mock_cancel_scan.assert_called_once()
