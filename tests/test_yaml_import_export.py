...
def test_parse_yaml_content_dict():
    """Verify that parse_yaml_content can parse a single dictionary."""
    yaml_input = """path: b.py
line: '2'
own_conf: 50%
"""
    results = gptscan.parse_yaml_content(yaml_input)
    ...
    assert results[0]["own_conf"] == "50%"

def test_parse_yaml_content_invalid():
    """Verify that parse_yaml_content handles invalid syntax and types gracefully."""
    # Invalid YAML type (e.g. integer)
    assert gptscan.parse_yaml_content("123") == []

    # Malformed YAML syntax raises ValueError
    malformed_yaml = "path: [unclosed"
...
def test_import_results_yaml_success(monkeypatch, tmp_path):
    ...
    yaml_content = """- path: imported_script.py
  line: '10'
  own_conf: 45%
  gpt_conf: 55%
...
def test_yaml_missing_package(monkeypatch):
    """Verify that when pyyaml package is missing, we raise appropriate errors."""
    import builtins
    ...
    with pytest.raises(ImportError, match="PyYAML is required"):
        gptscan.parse_yaml_content("foo: bar")
