...
def test_generate_yaml_basic():
    """Verify that generate_yaml correctly outputs standard structured YAML."""
    if not gptscan.HAS_YAML:
...
def test_parse_yaml_content_basic():
...
def test_parse_yaml_content_invalid():
    """Verify that parse_yaml_content handles invalid YAML safely."""
    if not gptscan.HAS_YAML:
        pytest.skip("pyyaml is not installed")
...
"""
    # 1. Auto-detect from format (contains ':' but doesn't start with { or [ or <)
