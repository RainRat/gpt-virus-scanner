...
def test_scan_files_deep_scan_coverage(monkeypatch, tmp_path, mock_scan_dependencies, file_size, expected_calls):
    ...
    monkeypatch.setattr(gptscan, "collect_files", lambda targets, exclude_patterns=None: [test_file])
...
def test_scan_files_metadata_error(monkeypatch, tmp_path, mock_scan_dependencies):
    ...
    monkeypatch.setattr(gptscan, "collect_files", lambda targets, exclude_patterns=None: [mock_path])
