...
def generate_console_report(results: List[Dict[str, Any]], use_color: bool = False) -> str:
    ...
    def colorize_conf(conf_str):
        if not use_color:
            return conf_str
        val = parse_percent(conf_str)
        if val >= 80:
            return f"{RED}{conf_str}{RESET}"
        if val >= 50:
            return f"{YELLOW}{conf_str}{RESET}"
    ...
    for i, r in enumerate(results, 1):
        ...
        meta_parts = [f"{GRAY}Local:{RESET} {colorize_conf(own_conf)}"]
        if gpt_conf:
            meta_parts.append(f"{GRAY}AI:{RESET} {colorize_conf(gpt_conf)}")
        ...
        for sl in snippet_lines:
            clean_sl = sl.rstrip()
            if len(clean_sl) > 100:
                clean_sl = clean_sl[:97] + "..."
            lines.append(f"    {GRAY}> {clean_sl}{RESET}")
        lines.append("")
