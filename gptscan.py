...
def analyze_filename(path: Union[str, Path]) -> Tuple[float, str]:
    ...
    if any(name.lower().endswith(ext) for ext in exec_extensions):
        # Check if there's a large gap before the extension
        # e.g. "invoice.pdf                              .exe"
        # Supports both spaces and tabs in the gap.
        if re.search(r'\s{5,}\.[^.]+$', name):
    ...
    if name and (name[-1].isspace() or name.endswith('.')):
        return 0.5, "Filename ends with a suspicious whitespace or dot, often used to bypass filters."
