...
def view_online(event_or_path: Union[tk.Event, str, None] = None, line: Optional[Union[int, str]] = None) -> None:
    ...
    if isinstance(event_or_path, str):
        ...
        for item_id in selection:
            ...
            if vals:
                path = str(vals[0])
                line_num = line if line is not None else (vals[6] if len(vals) > 6 else 1)
    ...
    if found_count > 0:
        if found_count == 1 and len(targets) == 1:
            update_status(f"Opening online view for {os.path.basename(targets[0][0])}...")
        else:
            unit = "file" if found_count == 1 else "files"
            update_status(f"Opening online view for {found_count} {unit}...")
    elif not isinstance(event_or_path, str):
        messagebox.showinfo(
