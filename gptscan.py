...
def browse_url_click() -> None:
    """Handle the URL input dialog and populate the textbox."""
    url = simpledialog.askstring("Scan URL", "Enter the URL of a script to scan:")
    if url:
        _set_scan_target(url.strip())
...
def create_gui(initial_path: Optional[str] = None) -> tk.Tk:
    ...
    select_url_btn = ttk.Button(input_frame, text="URL...", command=browse_url_click)
    select_url_btn.grid(row=0, column=4, sticky="e", padx=(5, 0), ipady=5)
    bind_hover_message(select_url_btn, "Enter a URL of a script to scan.")
