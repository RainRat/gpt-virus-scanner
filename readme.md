...
#### CI/CD & Exit Codes
Run the scanner in CI/CD pipelines (like GitHub Actions) to block dangerous code before merging.

Set `--fail-threshold` with a threat level from 0 to 100. If any file reaches or exceeds this level, the scanner exits with code `1` and stops the pipeline:
```bash
# Fail the build if any file has a threat level of 70 or higher
python3 gptscan.py ./my_project --cli --fail-threshold 70
```

Here is an example GitHub Actions configuration (`.github/workflows/scan.yml`):
```yaml
name: Security Scan
