...
To sort output findings by threat score, file path, or line number, use `--sort-by` (or `--sort`). You can also reverse the sort order using `-r` (or `--reverse`):
```bash
...
*   `-r`, `--reverse`: Reverse the order of the output scan results.
