...
*   `--baseline <file>`: Use a previous scan report (in any supported format) as a baseline to filter out existing findings.
...
*   **Exclusions:** Ignore specific files or folders by using **File > Manage Exclusions...** or by adding patterns to a `.gptscanignore` file. In the terminal, use the `-e` or `--exclude` flag, or load patterns from a file using `--exclude-file`.
